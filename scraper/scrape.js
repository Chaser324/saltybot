...
var baseUrl, betStatusBlueWins, betStatusClosed, betStatusOpen, betStatusRedWins, blueBet, blueFigher, casper, currentTournament, f, loginUrl, matchLength, matchStartTime, redBet, redFigher, userinfo, utils, wagerAmount, wagerFigther;
...
casper.then(function() {
  redFigher = this.evaluate(function() {
    return $('#p1name').text();
  });
  blueFigher = this.evaluate(function() {
    return $('#p2name').text();
  });
  wagerAmount = 10;
  wagerFigther = redFigher;
  this.echo('Current Match: ' + redFigher + ' vs ' + blueFigher);
  this.echo('Betting ' + wagerAmount + ' on ' + wagerFigther);
  this.evaluate((function(amount) {
    return $('#wager').val(amount);
  }), wagerAmount);
  if (wagerFigther === redFigher) {
    return this.click('input.betbuttonred');
  } else if (wagerFigther === blueFigher) {
